...
const drawLine = ({ width, height, field, setSelected }) => {
  ...
  const scaleY = scaleLinear()
    .domain([
      min(dataset.map((it) => it[field])),
      max(dataset.map((it) => it[field])),
    ])
    .range([height - Y_MARGIN * 2, Y_MARGIN])

  const scaleX = scaleTime()
    .domain([
      min(dataset.map((it) => +it.date)),
      max(dataset.map((it) => +it.date)),
    ])
  ...
  ;(chartPath.empty()
    ? select('#chart').append('path').attr('class', 'path')
    : chartPath
  )
    .datum(dataset)
    .attr('stroke', 'white')
  ...
  const selectionShadow = select('#chart')
